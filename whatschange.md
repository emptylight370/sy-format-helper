## [0.4.5](https://github.com/emptylight370/sy-format-helper/compare/0.4.4...0.4.5) (2026-02-17)

### ✨ Features | 功能

- 在发布更新时生成更新日志 ([29928f1](https://github.com/emptylight370/sy-format-helper/commit/29928f13d2b84cdc3460087c8dbbddab790abcaf))
- Update deps ([9ea99fe](https://github.com/emptylight370/sy-format-helper/commit/9ea99fe5c1a2400570f576e2f6240ec9e20b30d8))
- Update deps ([d529b18](https://github.com/emptylight370/sy-format-helper/commit/d529b186914909c2354a9d056c590b2556e32510))

### 📄 Documentation | 文档

- 更新安装说明 ([3e11ece](https://github.com/emptylight370/sy-format-helper/commit/3e11eceee4a9dfdbbad5cf123494ef4c4d0f8397))
- 将下载数量改为需要帮助的议题数量 ([38fff3e](https://github.com/emptylight370/sy-format-helper/commit/38fff3eb653c20a12088903587eeeff505f39023))

### 🛠️ Build system | 构建系统

- 更新工作流和项目依赖 ([a279c46](https://github.com/emptylight370/sy-format-helper/commit/a279c462bbaecf7ea5b67234c35fef5320f2df56))
- 更新依赖环境 ([23ea2ae](https://github.com/emptylight370/sy-format-helper/commit/23ea2ae0189ce15fab299fd9e662d14e40a1d1a9))
- 迁移到mise管理，更新GitHub Actions ([cd0e4e5](https://github.com/emptylight370/sy-format-helper/commit/cd0e4e5f39c4347304b6bc2553c9801b23c99f1b))
- 添加Prettier格式化TOML文件的配置 ([cdfef4d](https://github.com/emptylight370/sy-format-helper/commit/cdfef4d0d09f12a0ef2c0d81922e6bc8de807cd2))
- 在提交前格式化文件 ([70bc8f8](https://github.com/emptylight370/sy-format-helper/commit/70bc8f8d1343c9cb813330c55186ccc6089cdb6d))
- **action:** 更新GitHub Actions以适应变化 ([052c579](https://github.com/emptylight370/sy-format-helper/commit/052c5791644371d10b3ce91e21c64b95072a20d8))
- **lefthook:** 并行执行检查缩短处理时间 ([042cb5c](https://github.com/emptylight370/sy-format-helper/commit/042cb5cc42a7788a794cca6bd9d725cff62a9d78))
- **lefthook:** 修改触发条件 ([8940d54](https://github.com/emptylight370/sy-format-helper/commit/8940d547063eb651ab50ba2b59542441362579ca))
- **lefthook:** 重置格式化文件的逻辑 ([8744685](https://github.com/emptylight370/sy-format-helper/commit/87446850ef43ce5521fd93b57831e2f5f0e0fb58))
- Update workflow ([ee1cc7d](https://github.com/emptylight370/sy-format-helper/commit/ee1cc7d372441d75234075b75b767857d0dbe2cd))

## [0.4.4](https://github.com/emptylight370/sy-format-helper/compare/0.4.3...0.4.4) (2025-09-08)

### ⚡ Performance | 性能优化

- Disable in publish mode ([da8a4cb](https://github.com/emptylight370/sy-format-helper/commit/da8a4cb04e775b60a1bd9c5f0db7e99721442401))

### 🔨 Refactor | 重构

- 迁移到Volta ([0e709a4](https://github.com/emptylight370/sy-format-helper/commit/0e709a49e3e9abea46e43528e1169830a495254c))

### 🛠️ Build system | 构建系统

- 切换jq到现成动作 ([a410c61](https://github.com/emptylight370/sy-format-helper/commit/a410c61afc445f37b1b7b0bce191fdba05734768))
- 修改node版本至最新 ([fdcffcc](https://github.com/emptylight370/sy-format-helper/commit/fdcffcc3d7a8517e108cf5fba4333531fc9d2b4e))

## [0.4.3](https://github.com/emptylight370/sy-format-helper/compare/0.4.2...0.4.3) (2025-08-18)

### ✨ Features | 功能

- 思源消息增加插件名称显示 ([eb8afd6](https://github.com/emptylight370/sy-format-helper/commit/eb8afd6432766935b0439a056b7177da41ea3627))

### 📄 Documentation | 文档

- Add install guide ([39aee1c](https://github.com/emptylight370/sy-format-helper/commit/39aee1c06a53f4d43a46392d930f6886a403c304))

### 🛠️ Build system | 构建系统

- 更新工作流以修复问题 ([977723e](https://github.com/emptylight370/sy-format-helper/commit/977723e4e0c6437f13e6ca726062145f11ae0e49))

## [0.4.2](https://github.com/emptylight370/sy-format-helper/compare/0.4.1...0.4.2) (2025-04-16)

### ✨ Features | 功能

- 添加了法律法规条目后增加空格功能 ([9fb07be](https://github.com/emptylight370/sy-format-helper/commit/9fb07be1badbed24864964b640663138c2a467e1))

## [0.4.1](https://github.com/emptylight370/sy-format-helper/compare/0.4.0...0.4.1) (2025-03-24)

### ✨ Features | 功能

- 添加全半角字符互转功能 ([63b2fb8](https://github.com/emptylight370/sy-format-helper/commit/63b2fb861d73db079b03f0c43a16e7fab25fd7b0))

## [0.4.0](https://github.com/emptylight370/sy-format-helper/compare/0.3.2...0.4.0) (2025-03-24)

### ✨ Features | 功能

- 代码块添加移除缩进按钮 ([0b75f24](https://github.com/emptylight370/sy-format-helper/commit/0b75f24b11565f538e20c8fd3610af13328e68e4))
- 现在能够一次处理多个块 ([300cc7e](https://github.com/emptylight370/sy-format-helper/commit/300cc7e90ea5ef62b58050f7ab911dc18d244c3e))
- 修改正则表达式，智能去除空格 ([da3a87a](https://github.com/emptylight370/sy-format-helper/commit/da3a87af3bdd9700c99baae4accb57f0b2201e45))
- Add local scripts ([8d04943](https://github.com/emptylight370/sy-format-helper/commit/8d0494387c865d899c9e5f40fb829c0810ba585c))

## [0.3.2](https://github.com/emptylight370/sy-format-helper/compare/0.3.1...0.3.2) (2025-02-09)

### 📄 Documentation | 文档

- Update Readme ([ad7e0a7](https://github.com/emptylight370/sy-format-helper/commit/ad7e0a7b5825d7fe3de76501a6c17ba658431add))

## [0.3.1](https://github.com/emptylight370/sy-format-helper/compare/0.3.0...0.3.1) (2025-02-06)

### ✨ Features | 功能

- 添加全大写和全小写功能 ([9412e6f](https://github.com/emptylight370/sy-format-helper/commit/9412e6f99394e2f5b1e7a5eefe7453dab87ac38e))

### 📄 Documentation | 文档

- 更新日志 ([b3c71d8](https://github.com/emptylight370/sy-format-helper/commit/b3c71d8926ffa1735182b08f40070a3fe175149c))
- 更新文档 ([3465516](https://github.com/emptylight370/sy-format-helper/commit/34655163dc023ad1a7e7dde2e79de4533195f56c))
- docs update ([8c71f68](https://github.com/emptylight370/sy-format-helper/commit/8c71f68391b03f8319455b685a0425d48c8a9161))

### 🔨 Refactor | 重构

- **deps:** 更新依赖库：siyuan ([480f35c](https://github.com/emptylight370/sy-format-helper/commit/480f35c8835c5622c5d10c908b2c482fae7f16d6))

## [0.3.0](https://github.com/emptylight370/sy-format-helper/compare/0.2.5...0.3.0) (2024-12-24)

### ✨ Features | 功能

- 添加几个统计标签 ([b8c1bf6](https://github.com/emptylight370/sy-format-helper/commit/b8c1bf66c54703f0e0907fa632296c2c885d2970))

### 🐛 Bug Fixes | 问题修复

- **core:** 修改部分api调用[#1](https://github.com/emptylight370/sy-format-helper/issues/1) ([a1a57ca](https://github.com/emptylight370/sy-format-helper/commit/a1a57ca47791e137c73f3d0ba5407538aafb9962))

### 📄 Documentation | 文档

- 将下载量统计改为最新版本的下载量 ([4722456](https://github.com/emptylight370/sy-format-helper/commit/4722456b93a23633357a2672ce54af7fb2797a7d))

## [0.2.5](https://github.com/emptylight370/sy-format-helper/compare/0.2.4...0.2.5) (2024-12-23)

### 🐛 Bug Fixes | 问题修复

- **core:** 意外处理tag ([88e7c50](https://github.com/emptylight370/sy-format-helper/commit/88e7c5010a7aab06b0ac11feb3e1e11b683bc79b))

## [0.2.4](https://github.com/emptylight370/sy-format-helper/compare/0.2.3...0.2.4) (2024-12-23)

### 📄 Documentation | 文档

- Update README ([f134a43](https://github.com/emptylight370/sy-format-helper/commit/f134a43cc306f81419ade56f9f5694ed62cfc27a))

## [0.2.3](https://github.com/emptylight370/sy-format-helper/compare/0.2.2...0.2.3) (2024-12-22)

### 📄 Documentation | 文档

- 更新说明文档 ([47f1461](https://github.com/emptylight370/sy-format-helper/commit/47f14612c6d198e81f10eb66dadde807b4489149))

## [0.2.2](https://github.com/emptylight370/sy-format-helper/compare/0.2.1...0.2.2) (2024-12-22)

### ✨ Features | 功能

- **action:** 添加议题模板 ([ab4f33b](https://github.com/emptylight370/sy-format-helper/commit/ab4f33b602a8ea6a05fd76b3178e8352a6b66b9f))

### 🐛 Bug Fixes | 问题修复

- **core:** 添加空格的文本匹配错误 ([90b6f89](https://github.com/emptylight370/sy-format-helper/commit/90b6f89eeaeeb3d9bb973e678378b3f0a1285439))

### 📄 Documentation | 文档

- 文档更新 ([c11b597](https://github.com/emptylight370/sy-format-helper/commit/c11b59723fe51cb83a1161b682e6a329448c472e))

## [0.2.1](https://github.com/emptylight370/sy-format-helper/compare/0.2.0...0.2.1) (2024-12-21)

### ✨ Features | 功能

- **core:** 更新处理方法 ([770df47](https://github.com/emptylight370/sy-format-helper/commit/770df472541a4fc2fc187912fbc1a807400cc2ec))
- **core:** 实现在数字和英文前后添加空格功能 ([3fdf36a](https://github.com/emptylight370/sy-format-helper/commit/3fdf36a4732641a8aac8aaeb1cef3146888c31c0))
- **core:** 添加不同的选中块类型判断 ([e1a6597](https://github.com/emptylight370/sy-format-helper/commit/e1a6597bd704f938c1f09c00970dd1ef0086be7e))

## [0.2.0](https://github.com/emptylight370/sy-format-helper/compare/0.1.1...0.2.0) (2024-12-20)

### ✨ Features | 功能

- **core:** 将替换操作改写成dom模式 ([a1d956f](https://github.com/emptylight370/sy-format-helper/commit/a1d956fd633f6c4d2f37eb7028f8e30077640df7))

### 📄 Documentation | 文档

- fix ([cb7512c](https://github.com/emptylight370/sy-format-helper/commit/cb7512c2e89a9a139e8f8b662c1871973fff7d71))

## [0.1.1](https://github.com/emptylight370/sy-format-helper/compare/0.1.0...0.1.1) (2024-12-17)

### ⚠ BREAKING CHANGES

- **core:** 位于行首的标签可能会因为对标题的处理而变为标题，未测试

### ✨ Features | 功能

- **core:** 优化代码实现 ([49d6cd4](https://github.com/emptylight370/sy-format-helper/commit/49d6cd4f40ffb0218e4998efa87076865c5fee41))

### 🐛 Bug Fixes | 问题修复

- **core:** 块引用的前面如果是文本也加上空格 ([cfa778e](https://github.com/emptylight370/sy-format-helper/commit/cfa778e228e235d2f4b1f07e0674575f2f34175f))

### 📄 Documentation | 文档

- 填充说明文档 ([2d2f067](https://github.com/emptylight370/sy-format-helper/commit/2d2f067bc997ba06e7e96d8c5893fab28b8e5dd2))

## [0.1.0](https://github.com/emptylight370/sy-format-helper/compare/0f5eaafcb4a14fe82de1c171f4af8cd01cbe0f6f...0.1.0) (2024-12-17)

### ✨ Features | 功能

- **actions:** 在环境里安装pnpm ([778088a](https://github.com/emptylight370/sy-format-helper/commit/778088abac28f410a34ed2534505dc5af314f636))
- **Actions:** add package workflow ([d4bb11c](https://github.com/emptylight370/sy-format-helper/commit/d4bb11c053609a834f3a77b037c6297726306582))
- **core:** Success remove whitespaces ([5b9be6b](https://github.com/emptylight370/sy-format-helper/commit/5b9be6bd777602f19a61ceb072515dc8ba72e370))
- **pnpm:** 添加命令方便调试 ([b53a51e](https://github.com/emptylight370/sy-format-helper/commit/b53a51e80d563c5c09ff2cab5602523a54322b4e))

### 📄 Documentation | 文档

- **plugin.json:** config init ([0f5eaaf](https://github.com/emptylight370/sy-format-helper/commit/0f5eaafcb4a14fe82de1c171f4af8cd01cbe0f6f))
